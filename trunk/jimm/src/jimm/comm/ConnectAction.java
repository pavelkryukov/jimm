		...
		try
		{
			...
			else if (this.state == ConnectAction.STATE_CLI_COOKIE_SENT)
			{
				...
				if (packet instanceof SnacPacket)
				{
					    ...
					    (snacPacket.getCommand() == SnacPacket.SRV_FAMILIES_COMMAND))
					{
						...
						SnacPacket reply = new SnacPacket(SnacPacket.CLI_FAMILIES_FAMILY, SnacPacket.CLI_FAMILIES_COMMAND,
						                                  0x00000000, new byte[0], ConnectAction.CLI_FAMILIES_DATA);
						this.icq.c.sendPacket(reply);
			...
			else if (this.state == ConnectAction.STATE_CLI_RATESREQUEST_SENT)
			{
				...
				if (packet instanceof SnacPacket)
				{
					    ...
					    (snacPacket.getCommand() == SnacPacket.SRV_RATES_COMMAND))
					{
						...
						SnacPacket reply1 = new SnacPacket(SnacPacket.CLI_ACKRATES_FAMILY, SnacPacket.CLI_ACKRATES_COMMAND,
						                                   0x00000000, new byte[0], ConnectAction.CLI_ACKRATES_DATA);
						this.icq.c.sendPacket(reply1);

						// Send a CLI_REQLOCATION packet
						SnacPacket reply2 = new SnacPacket(SnacPacket.CLI_REQLOCATION_FAMILY, SnacPacket.CLI_REQLOCATION_COMMAND,
						                                   0x00000000, new byte[0], new byte[0]);
						this.icq.c.sendPacket(reply2);

						// Send a CLI_SETUSERINFO packet
						SnacPacket reply3 = new SnacPacket(SnacPacket.CLI_SETUSERINFO_FAMILY, SnacPacket.CLI_SETUSERINFO_COMMAND,
						                                   0x00000000, new byte[0], ConnectAction.CLI_SETUSERINFO_DATA);
						this.icq.c.sendPacket(reply3);

						// Send a CLI_REQBUDDY packet
						SnacPacket reply4 = new SnacPacket(SnacPacket.CLI_REQBUDDY_FAMILY, SnacPacket.CLI_REQBUDDY_COMMAND,
						                                   0x00000000, new byte[0], new byte[0]);
						this.icq.c.sendPacket(reply4);

						// Send a CLI_REQICBM packet
						SnacPacket reply5 = new SnacPacket(SnacPacket.CLI_REQICBM_FAMILY, SnacPacket.CLI_REQICBM_COMMAND,
						                                   0x00000000, new byte[0], new byte[0]);
						this.icq.c.sendPacket(reply5);

						// Send a CLI_REQBOS packet
						SnacPacket reply6 = new SnacPacket(SnacPacket.CLI_REQBOS_FAMILY, SnacPacket.CLI_REQBOS_COMMAND,
						                                   0x00000000, new byte[0], new byte[0]);
						this.icq.c.sendPacket(reply6);

						// Send a CLI_REQLISTS packet
						SnacPacket reply7 = new SnacPacket(SnacPacket.CLI_REQLISTS_FAMILY, SnacPacket.CLI_REQLISTS_COMMAND,
						                                   0x00000000, new byte[0], new byte[0]);
			...
			else if (this.state == ConnectAction.STATE_CLI_REQLISTS_SENT)
			{
				...
				if (packet instanceof SnacPacket)
				{
					...
					if (this.srvReplyLocationRcvd && this.srvReplyBuddyRcvd && this.srvReplyIcbmRcvd &&
					    this.srvReplyBosRcvd && this.srvReplyListsRcvd)
					{

						// Send a CLI_SETICBM packet
						SnacPacket reply1 = new SnacPacket(SnacPacket.CLI_SETICBM_FAMILY, SnacPacket.CLI_SETICBM_COMMAND,
						                                   0x00000000, new byte[0], ConnectAction.CLI_SETICBM_DATA);
						this.icq.c.sendPacket(reply1);
						...
						else
						{
							...
							SnacPacket reply2 = new SnacPacket(SnacPacket.CLI_CHECKROSTER_FAMILY, SnacPacket.CLI_CHECKROSTER_COMMAND,
			...
			else if (this.state == ConnectAction.STATE_CLI_CHECKROSTER_SENT)
			{
				...
				if (packet instanceof SnacPacket)
				{
					...
					if (this.srvReplyRosterRcvd)
					{
						...
						SnacPacket reply1 = new SnacPacket(SnacPacket.CLI_ROSTERACK_FAMILY, SnacPacket.CLI_ROSTERACK_COMMAND,
						                                   0x00000000, new byte[0], new byte[0]);
						this.icq.c.sendPacket(reply1);
						...
						SnacPacket reply2 = new SnacPacket(SnacPacket.CLI_SETSTATUS_FAMILY, SnacPacket.CLI_SETSTATUS_COMMAND,
						                                   0x00000000, new byte[0], ConnectAction.CLI_SETSTATUS_DATA);
						this.icq.c.sendPacket(reply2);

						// Send a CLI_READY packet
						SnacPacket reply3 = new SnacPacket(SnacPacket.CLI_READY_FAMILY, SnacPacket.CLI_READY_COMMAND,
						                                   0x00000000, new byte[0], ConnectAction.CLI_READY_DATA);
						this.icq.c.sendPacket(reply3);

						// Send a CLI_TOICQSRV/CLI_REQOFFLINEMSGS packet
						ToIcqSrvPacket reply4 = new ToIcqSrvPacket(0x00000000, this.uin, ToIcqSrvPacket.CLI_REQOFFLINEMSGS_SUBCMD,
