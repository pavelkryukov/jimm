...
{
	// Client CAPS
	public static final byte[] CAP_AIM_SERVERRELAY = explodeToBytes(
			"09,46,13,49,4C,7F,11,D1,82,22,44,45,53,54,00,00", ',', 16);

	public static final byte[] CAP_UTF8 = explodeToBytes(
			"09,46,13,4E,4C,7F,11,D1,82,22,44,45,53,54,00,00", ',', 16);

	public static final byte[] CAP_UTF8_GUID = explodeToBytes(
			"7b,30,39,34,36,31,33,34,45,2D,34,43,37,46,2D,31,31,44,31,2D,38,32,32,32,2D,34,34,34,35,35,33,35,34,30,30,30,30,7D",
			',', 16);

	private static final byte[] CAP_MIRANDAIM = explodeToBytes(
			"4D,69,72,61,6E,64,61,4D,00,00,00,00,00,00,00,00", ',', 16);

	private static final byte[] CAP_TRILLIAN = explodeToBytes(
			"97,b1,27,51,24,3c,43,34,ad,22,d6,ab,f7,3f,14,09", ',', 16);

	private static final byte[] CAP_TRILCRYPT = explodeToBytes(
			"f2,e7,c7,f4,fe,ad,4d,fb,b2,35,36,79,8b,df,00,00", ',', 16);

	private static final byte[] CAP_SIM = explodeToBytes(
			"*SIM client  ,0,0,0,0", ',', 16);

	private static final byte[] CAP_SIMOLD = explodeToBytes(
			"97,b1,27,51,24,3c,43,34,ad,22,d6,ab,f7,3f,14,00", ',', 16);

	private static final byte[] CAP_LICQ = explodeToBytes(
			"*Licq client ',0,0,0,0", ',', 16);

	private static final byte[] CAP_KOPETE = explodeToBytes(
			"*Kopete ICQ  ',0,0,0,0", ',', 16);

	private static final byte[] CAP_MICQ = explodeToBytes(
			"*mICQ ,A9,* R.K. ',0,0,0,0", ',', 16);

	private static final byte[] CAP_ANDRQ = explodeToBytes(
			"*&RQinside,0,0,0,0,0,0,0", ',', 16);

	private static final byte[] CAP_QIP = explodeToBytes(
			"56,3F,C8,09,0B,6F,41,*QIP 2005a", ',', 16);

	private static final byte[] CAP_IM2 = explodeToBytes(
			"74,ED,C3,36,44,DF,48,5B,8B,1C,67,1A,1F,86,09,9F", ',', 16);

	private static final byte[] CAP_MACICQ = explodeToBytes(
			"dd,16,f2,02,84,e6,11,d4,90,db,00,10,4b,9b,4b,7d", ',', 16);

	private static final byte[] CAP_RICHTEXT = explodeToBytes(
			"97,b1,27,51,24,3c,43,34,ad,22,d6,ab,f7,3f,14,92", ',', 16);

	private static final byte[] CAP_IS2001 = explodeToBytes(
			"2e,7a,64,75,fa,df,4d,c8,88,6f,ea,35,95,fd,b6,df", ',', 16);

	private static final byte[] CAP_IS2002 = explodeToBytes(
			"10,cf,40,d1,4c,7f,11,d1,82,22,44,45,53,54,00,00", ',', 16);

	private static final byte[] CAP_STR20012 = explodeToBytes(
			"a0,e9,3f,37,4f,e9,d3,11,bc,d2,00,04,ac,96,dd,96", ',', 16);

	private static final byte[] CAP_AIMICON = explodeToBytes(
			"09,46,13,46,4c,7f,11,d1,82,22,44,45,53,54,00,00", ',', 16);

	private static final byte[] CAP_AIMIMIMAGE = explodeToBytes(
			"09,46,13,45,4c,7f,11,d1,82,22,44,45,53,54,00,00", ',', 16);

	private static final byte[] CAP_AIMCHAT = explodeToBytes(
			"74,8F,24,20,62,87,11,D1,82,22,44,45,53,54,00,00", ',', 16);

	private static final byte[] CAP_UIM = explodeToBytes(
			"A7,E4,0A,96,B3,A0,47,9A,B8,45,C9,E4,67,C5,6B,1F", ',', 16);

	private static final byte[] CAP_RAMBLER = explodeToBytes(
			"7E,11,B7,78,A3,53,49,26,A8,02,44,73,52,08,C4,2A", ',', 16);

	private static final byte[] CAP_ABV = explodeToBytes(
			"00,E7,E0,DF,A9,D0,4F,e1,91,62,C8,90,9A,13,2A,1B", ',', 16);

	private static final byte[] CAP_NETVIGATOR = explodeToBytes(
			"4C,6B,90,A3,3D,2D,48,0E,89,D6,2E,4B,2C,10,D9,9F", ',', 16);

	private static final byte[] CAP_XTRAZ = explodeToBytes(
			"1A,09,3C,6C,D7,FD,4E,C5,9D,51,A6,47,4E,34,F5,A0", ',', 16);

	private static final byte[] CAP_AIMFILE = explodeToBytes(
			"09,46,13,43,4C,7F,11,D1,82,22,44,45,53,54,00,00", ',', 16);

	private static final byte[] CAP_DIRECT = explodeToBytes(
			"09,46,13,44,4C,7F,11,D1,82,22,44,45,53,54,00,00", ',', 16);

	private static final byte[] CAP_JIMM = explodeToBytes("*Jimm ", ',', 16);

	private static final byte[] CAP_AVATAR = explodeToBytes(
			"09,46,13,4C,4C,7F,11,D1,82,22,44,45,53,54,00,00", ',', 16);

	private static final byte[] CAP_TYPING = explodeToBytes(
			"56,3f,c8,09,0b,6f,41,bd,9f,79,42,26,09,df,a2,f3", ',', 16);

	private static final byte[] CAP_MCHAT = explodeToBytes("*mChat icq", ',',
			16);
	...
	private static final byte[] CAP_OLD_TAIL = explodeToBytes(
			"4C,7F,11,D1,82,22,44,45,53,54,00,00", ',', 16);
	...
	public static void detectUserClient(String uin, int dwFP1, int dwFP2,
			int dwFP3, byte[] capabilities, int wVersion, boolean statusChange)
	{
		...
		if (item != null)
		{
			if (capabilities != null)
			{
				...
				{
					int j16 = j * 16;
					if (Util.byteArrayEquals(capabilities, j16,
							CAP_AIM_SERVERRELAY, 0, 16))
					{
						caps |= CAPF_AIM_SERVERRELAY_INTERNAL;
					} else if (Util.byteArrayEquals(capabilities, j16,
							CAP_UTF8, 0, 16))
					{
					...
					{
						caps |= CAPF_MIRANDAIM;
						szVersion = detectClientVersion(uin, capabilities,
								CAPF_MIRANDAIM, j);
					} else if (Util.byteArrayEquals(capabilities, j16,
							CAP_TRILLIAN, 0, 16))
					{
					...
					{
						caps |= CAPF_TRILCRYPT;
					} else if (Util.byteArrayEquals(capabilities, j16, CAP_SIM,
							0, 0xC))
					{
						caps |= CAPF_SIM;
					} else if (Util.byteArrayEquals(capabilities, j16,
							...
							CAP_ANDRQ, 0, 9))
					{
						...
						szVersion = detectClientVersion(uin, capabilities,
								CAPF_ANDRQ, j);
					} else if (Util.byteArrayEquals(capabilities, j16, CAP_QIP,
							0, 11))
					{
						caps |= CAPF_QIP;
						szVersion = detectClientVersion(uin, capabilities,
								CAPF_QIP, j);
					} else if (Util.byteArrayEquals(capabilities, j16, CAP_IM2,
							0, 16))
					{
						caps |= CAPF_IM2;
					} else if (Util.byteArrayEquals(capabilities, j16,
					...
					{
						caps |= CAPF_AIMCHAT;
					} else if (Util.byteArrayEquals(capabilities, j16, CAP_UIM,
							0, 16))
					{
						caps |= CAPF_UIM;
					} else if (Util.byteArrayEquals(capabilities, j16,
					...
					{
						caps |= CAPF_RAMBLER;
					} else if (Util.byteArrayEquals(capabilities, j16, CAP_ABV,
							0, 16))
					{
						caps |= CAPF_ABV;
					} else if (Util.byteArrayEquals(capabilities, j16,
							...
							CAP_JIMM, 0, 5))
					{
						...
						caps |= CAPF_DIRECT;
							...
							CAP_TYPING, 0, 16))
						caps |= CAPF_TYPING;
					else if (Util.byteArrayEquals(capabilities, j16, CAP_MCHAT,
							0, 9))
					{
						caps |= CAPF_MCHAT;
						szVersion = detectClientVersion(uin, capabilities,
				...
				item.setIntValue(
						ContactListContactItem.CONTACTITEM_CAPABILITIES, caps);
			}
			...
			if (!statusChange)
			{
				...
				if (client != CLI_NONE)
				{
					item.setIntValue(ContactListContactItem.CONTACTITEM_CLIENT,
							client);
					item.setStringValue(
							ContactListContactItem.CONTACTITEM_CLIVERSION,
							szVersion);
				}
			}
		}
