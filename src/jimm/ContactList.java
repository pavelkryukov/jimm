...
public class ContactList implements CommandListener {
	...
	// Main menu command
	private static Command mainMenuCommand = new Command(ResourceBundle.getString("jimm.res.Text", "menu"), Command.ITEM, 1);
	...
	// Returns the id number #1 which identifies (together with id number #2) the saved contact list version
	public long getVersionId1() {
		return (this.versionId1);
	}

	// Returns the id number #2 which identifies (together with id number #1) the saved contact list version
	...
	private synchronized int sortElement(int index) {
		...
		// Insertion sort (sort first by status - online/offline/temprary -, then by nick)
			...
			for (int i = 0; i < items.length; i++) {
				...
				if (items[i] instanceof ContactListContactItem) {
					...
					for (j = 0; i < cItems.size(); j++) {
						if (((ContactListContactItem) this.cItems.elementAt(j)).compareTo((ContactListContactItem) items[i]) >= 0) {
							break;
						}
					}
	...
	// Updates the client-side contact list (called when coming back from Message Display)
...
//	        //System.out.println(this.contactList.getImage(i).toString()+" "+ this.contactList.getString(i));
...
//	  }
	...
	public void refreshList(boolean noSort, boolean message, int position) {
		...
		// How long should the visible list be? this.contactList.size() for not hiding
		...
		if (message){
			...
			if (!focused) {
				...
				// Only change image if we have a postion for it and the new status it not offline and were
				// are not hiding offline contacts
				if (position != Integer.MAX_VALUE) {
					cItem = (ContactListContactItem) this.cItems.elementAt(position);
					if (!((cItem.getStatus() == STATUS_OFFLINE) && Jimm.jimm.getOptionsRef().isClHideOffline())) {
						//System.out.println("jimm:ContactList:refreshList: Image change at:" + position);
						// Update list item
		...
		if (contactList.size() > 0) {
			if (!focused && (this.currSelCItem != null) && (this.cItems.indexOf(this.currSelCItem) != -1)) {
				if ((this.cItems.elementAt(this.contactList.getSelectedIndex()) != this.currSelCItem) && !Jimm.jimm.getOptionsRef().isClHideOffline()) {
	...
	public void commandAction(Command c, Displayable d) {
		...
		// Contact item has been selected
			...
			ContactListContactItem currCItem = (ContactListContactItem) this.cItems.elementAt(this.contactList.getSelectedIndex());
