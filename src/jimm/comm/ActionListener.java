...
public class ActionListener
{
  ...
  public static final byte[] CAP_AIM_SERVERRELAY =  {(byte) 0x09, (byte) 0x46, (byte) 0x13, (byte) 0x49,
  ...
  public static final byte[] CAP_UTF8 = {(byte) 0x09, (byte) 0x46, (byte) 0x13, (byte) 0x4E,
                                         (byte) 0x4C, (byte) 0x7F, (byte) 0x11, (byte) 0xD1,
                                         (byte) 0x82, (byte) 0x22, (byte) 0x44, (byte) 0x45,
                                         (byte) 0x53, (byte) 0x54, (byte) 0x00, (byte) 0x00};
  ...
  protected void forward(Packet packet) throws JimmException
  {
    ...
    if (packet instanceof SnacPacket)
    {
      ...
      if ((snacPacket.getFamily() == SnacPacket.SRV_USERONLINE_FAMILY) && (snacPacket.getCommand() == SnacPacket.SRV_USERONLINE_COMMAND))
      {
      ...
      if ((snacPacket.getFamily() == SnacPacket.SRV_USEROFFLINE_FAMILY) && (snacPacket.getCommand() == SnacPacket.SRV_USEROFFLINE_COMMAND))
      {
      ...
      if ((snacPacket.getFamily() == SnacPacket.SRV_RECVMSG_FAMILY) && (snacPacket.getCommand() == SnacPacket.SRV_RECVMSG_COMMAND))
      {
        ...
        else if (format == 0x0002)
        {
          ...
          if (ackType != 0x0000) return;   // Only normal messages are supported yet
          msgMarker += 2;
          ...
          // Skip values up to (and including) SUB_MSG_TYPE2.UNKNOWN (before MSGTYPE)
          ...
          if (((msgType == 0x0001) || (msgType == 0x0004)) && (rawText.length > 1))
          {
            ...
            else
            {
              ...
              // Decode text; split text first, if delimiter could be found
              String urlText, url;
              if (delim != -1)
              {
                urlText = Util.crlfToCr(Util.byteArrayToString(rawText, 0, delim, isUtf8));
                url = Util.crlfToCr(Util.byteArrayToString(rawText, delim + 1, rawText.length - delim - 1, isUtf8));
              }
              else
              {
            ...
            ackMarker += 1;
            SnacPacket ackPacket = new SnacPacket(SnacPacket.CLI_ACKMSG_FAMILY, SnacPacket.CLI_ACKMSG_COMMAND, 0, new byte[0], ackBuf);
          ...
          else if (msgType == 0x001A)
          {
            ...
            if (plugin.equals("Send Web Page Address (URL)"))
            {
              ...
              // Acknowledge message
              byte[] ackBuf = new byte[10 + 1 + uinLen + 2 + 51 + 3 + 20 + 4 + (int) pluginLen + 19 + 4 + textLen];
              int ackMarker = 0;
              System.arraycopy(buf, 0, ackBuf, ackMarker, 10);
              ackMarker += 10;
              Util.putByte(ackBuf, ackMarker, uinLen);
              ...
              Util.putByte(ackBuf, ackMarker, 0x00);
              ackMarker += 1;
              System.arraycopy(msg2Buf, extDataStart, ackBuf, ackMarker, 20 + 4 + (int) pluginLen + 19 + 4 + textLen);
              SnacPacket ackPacket = new SnacPacket(SnacPacket.CLI_ACKMSG_FAMILY, SnacPacket.CLI_ACKMSG_COMMAND, 0, new byte[0], ackBuf);
              this.icq.c.sendPacket(ackPacket);
