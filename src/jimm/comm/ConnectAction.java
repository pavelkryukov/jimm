...
public class ConnectAction extends Action
{
    ...
    { 
            (byte) 0x00, (byte) 0x05, (byte) 0x00, (byte) 0x30, // 3 capabilities (48 bytes) follow
    ...
    { 
            (byte) 0x00, (byte) 0x06, (byte) 0x00, (byte) 0x04, (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x00, // Online status
        ...
        try
        {
            ...
            if (this.state == ConnectAction.STATE_INIT_DONE)
            {
                ...
                if (packet instanceof ConnectPacket)
                {
                    ...
                    if (connectPacket.getType() == ConnectPacket.SRV_CLI_HELLO)
                    {
                        ...
                        ConnectPacket reply = new ConnectPacket(this.uin, this.password, ResourceBundle.getCurrUiLanguage().toLowerCase(),
            ...
            else if (this.state == ConnectAction.STATE_CLI_COOKIE_SENT)
            {
                ...
                if (packet instanceof SnacPacket)
                {
                    ...
                    if ((snacPacket.getFamily() == SnacPacket.SRV_FAMILIES_FAMILY)
                            && (snacPacket.getCommand() == SnacPacket.SRV_FAMILIES_COMMAND))
                    {
                        // Send a CLI_FAMILIES packet as reply
                        SnacPacket reply = new SnacPacket(SnacPacket.CLI_FAMILIES_FAMILY, SnacPacket.CLI_FAMILIES_COMMAND, 0x00000000,
                                new byte[0], ConnectAction.CLI_FAMILIES_DATA);
                        this.icq.c.sendPacket(reply);
            ...
            else if (this.state == ConnectAction.STATE_CLI_FAMILIES_SENT)
            {
                ...
                if (packet instanceof SnacPacket)
                {
                    ...
                    if ((snacPacket.getFamily() == SnacPacket.SRV_FAMILIES2_FAMILY)
                            && (snacPacket.getCommand() == SnacPacket.SRV_FAMILIES2_COMMAND))
                    {
                    ...
                    if (this.srvFamilies2Rcvd && this.srvMotdRcvd)
                    {
                        ...
                        SnacPacket reply = new SnacPacket(SnacPacket.CLI_RATESREQUEST_FAMILY, SnacPacket.CLI_RATESREQUEST_COMMAND,
            ...
            else if (this.state == ConnectAction.STATE_CLI_RATESREQUEST_SENT)
            {
                ...
                {
                    SnacPacket snacPacket = (SnacPacket) packet;
                    if ((snacPacket.getFamily() == SnacPacket.SRV_RATES_FAMILY)
                            && (snacPacket.getCommand() == SnacPacket.SRV_RATES_COMMAND))
                    {
                        // Send a CLI_ACKRATES packet
                        SnacPacket reply1 = new SnacPacket(SnacPacket.CLI_ACKRATES_FAMILY, SnacPacket.CLI_ACKRATES_COMMAND, 0x00000000,
                                new byte[0], ConnectAction.CLI_ACKRATES_DATA);
                        this.icq.c.sendPacket(reply1);

                        // Send a CLI_REQINFO packet
                        SnacPacket reply2 = new SnacPacket(SnacPacket.CLI_REQINFO_FAMILY, SnacPacket.CLI_REQINFO_COMMAND, 0x00000000,
                                new byte[0], new byte[0]);
                        this.icq.c.sendPacket(reply2);

                        // Send a CLI_REQLISTS packet
                        SnacPacket reply3 = new SnacPacket(SnacPacket.CLI_REQLISTS_FAMILY, SnacPacket.CLI_REQLISTS_COMMAND, 0x00000000,
                                new byte[0], new byte[0]);
                        ...
                        {
                            SnacPacket reply4 = new SnacPacket(SnacPacket.CLI_REQROSTER_FAMILY, SnacPacket.CLI_REQROSTER_COMMAND,
                                    0x00000000, new byte[0], new byte[0]);
                        ...
                        } else
                        {
                            ...
                            SnacPacket reply4 = new SnacPacket(SnacPacket.CLI_CHECKROSTER_FAMILY, SnacPacket.CLI_CHECKROSTER_COMMAND,
                                    0x00000000, new byte[0], data);
                            this.icq.c.sendPacket(reply4);
                        }

                        // Send a CLI_REQLOCATION packet
                        SnacPacket reply5 = new SnacPacket(SnacPacket.CLI_REQLOCATION_FAMILY, SnacPacket.CLI_REQLOCATION_COMMAND,
                                0x00000000, new byte[0], new byte[0]);
                        this.icq.c.sendPacket(reply5);

                        // Send a CLI_REQBUDDY packet
                        SnacPacket reply6 = new SnacPacket(SnacPacket.CLI_REQBUDDY_FAMILY, SnacPacket.CLI_REQBUDDY_COMMAND, 0x00000000,
                                new byte[0], new byte[0]);
                        this.icq.c.sendPacket(reply6);

                        // Send a CLI_REQICBM packet
                        SnacPacket reply7 = new SnacPacket(SnacPacket.CLI_REQICBM_FAMILY, SnacPacket.CLI_REQICBM_COMMAND, 0x00000000,
                                new byte[0], new byte[0]);
                        this.icq.c.sendPacket(reply7);

                        // Send a CLI_REQBOS packet
                        SnacPacket reply8 = new SnacPacket(SnacPacket.CLI_REQBOS_FAMILY, SnacPacket.CLI_REQBOS_COMMAND, 0x00000000,
                                new byte[0], new byte[0]);
            ...
            else if (this.state == ConnectAction.STATE_CLI_REQBOS_SENT)
            {
                ...
                if (packet instanceof SnacPacket)
                {
                    ...
                    if ((snacPacket.getFamily() == SnacPacket.SRV_REPLYINFO_FAMILY)
                            && (snacPacket.getCommand() == SnacPacket.SRV_REPLYINFO_COMMAND))
                    {
                    ...
                    if ((snacPacket.getFamily() == SnacPacket.SRV_REPLYLOCATION_FAMILY)
                            && (snacPacket.getCommand() == SnacPacket.SRV_REPLYLOCATION_COMMAND))
                    {
                    ...
                    if ((snacPacket.getFamily() == SnacPacket.SRV_REPLYBUDDY_FAMILY)
                            && (snacPacket.getCommand() == SnacPacket.SRV_REPLYBUDDY_COMMAND))
                    {
                    ...
                    if ((snacPacket.getFamily() == SnacPacket.SRV_REPLYICBM_FAMILY)
                            && (snacPacket.getCommand() == SnacPacket.SRV_REPLYICBM_COMMAND))
                    {
                    ...
                    if ((snacPacket.getFamily() == SnacPacket.SRV_REPLYBOS_FAMILY)
                            && (snacPacket.getCommand() == SnacPacket.SRV_REPLYBOS_COMMAND))
                    {
                    ...
                    if ((snacPacket.getFamily() == SnacPacket.SRV_REPLYLISTS_FAMILY)
                            && (snacPacket.getCommand() == SnacPacket.SRV_REPLYLISTS_COMMAND))
                    {
                    ...
                    if ((snacPacket.getFamily() == SnacPacket.SRV_REPLYROSTEROK_FAMILY)
                    ...
                    if (this.srvReplyInfoRcvd && this.srvReplyLocationRcvd && this.srvReplyBuddyRcvd && this.srvReplyIcbmRcvd
                            && this.srvReplyBosRcvd && this.srvReplyListsRcvd && this.srvReplyRosterRcvd)
                    {
                        // Send a CLI_ROSTERACK packet
                        SnacPacket reply1 = new SnacPacket(SnacPacket.CLI_ROSTERACK_FAMILY, SnacPacket.CLI_ROSTERACK_COMMAND, 0x00000000,
                                new byte[0], new byte[0]);
                        this.icq.c.sendPacket(reply1);

                        // Send a CLI_SETUSERINFO packet
                        SnacPacket reply2 = new SnacPacket(SnacPacket.CLI_SETUSERINFO_FAMILY, SnacPacket.CLI_SETUSERINFO_COMMAND,
                                0x00000000, new byte[0], ConnectAction.CLI_SETUSERINFO_DATA);
                        this.icq.c.sendPacket(reply2);

                        // Send a CLI_SETICBM packet
                        SnacPacket reply3 = new SnacPacket(SnacPacket.CLI_SETICBM_FAMILY, SnacPacket.CLI_SETICBM_COMMAND, 0x00000000,
                                new byte[0], ConnectAction.CLI_SETICBM_DATA);
                        this.icq.c.sendPacket(reply3);
                        ...
                        SnacPacket reply5 = new SnacPacket(SnacPacket.CLI_READY_FAMILY, SnacPacket.CLI_READY_COMMAND, 0x00000000,
                                new byte[0], ConnectAction.CLI_READY_DATA);
                        this.icq.c.sendPacket(reply5);

                        // Send a CLI_TOICQSRV/CLI_REQOFFLINEMSGS packet
                        ToIcqSrvPacket reply6 = new ToIcqSrvPacket(0x00000000, this.uin, ToIcqSrvPacket.CLI_REQOFFLINEMSGS_SUBCMD,
                                new byte[0], new byte[0]);
                        this.icq.c.sendPacket(reply6);
            ...
            else if (this.state == ConnectAction.STATE_CLI_REQOFFLINEMSGS_SENT)
            {
                ...
                if (packet instanceof FromIcqSrvPacket)
                {
                    ...
                    if (fromIcqSrvPacket.getSubcommand() == FromIcqSrvPacket.SRV_DONEOFFLINEMSGS_SUBCMD)
                    {
                        ...
                        ToIcqSrvPacket reply = new ToIcqSrvPacket(0x00000000, this.uin, ToIcqSrvPacket.CLI_ACKOFFLINEMSGS_SUBCMD,
                                new byte[0], new byte[0]);
                        this.icq.c.sendPacket(reply);
