        ...
        try
        {
            ...
            if (this.state == ConnectAction.STATE_INIT_DONE)
            {
                ...
                if (packet instanceof ConnectPacket)
                {
                    ...
                    if (connectPacket.getType() == ConnectPacket.SRV_CLI_HELLO)
                    {
                        ...
                        ConnectPacket reply = new ConnectPacket(this.uin, this.password, ResourceBundle
                                .getCurrUiLanguage().toLowerCase(), ResourceBundle.getCurrUiLanguage().toLowerCase());
                ...
                if (packet instanceof SnacPacket)
                {
                            ...
                            && (snacPacket.getCommand() == SnacPacket.SRV_FAMILIES_COMMAND))
                    {
                        ...
                        SnacPacket reply = new SnacPacket(SnacPacket.CLI_FAMILIES_FAMILY,
                                SnacPacket.CLI_FAMILIES_COMMAND, 0x00000000, new byte[0],
            ...
            else if (this.state == ConnectAction.STATE_CLI_FAMILIES_SENT)
            {
                ...
                if (packet instanceof SnacPacket)
                {
                    ...
                    if ((snacPacket.getFamily() == SnacPacket.SRV_MOTD_FAMILY)
                            && (snacPacket.getCommand() == SnacPacket.SRV_MOTD_COMMAND))
                    {
                    ...
                    if (this.srvFamilies2Rcvd && this.srvMotdRcvd)
                    {
                        ...
                        SnacPacket reply = new SnacPacket(SnacPacket.CLI_RATESREQUEST_FAMILY,
                                SnacPacket.CLI_RATESREQUEST_COMMAND, 0x00000000, new byte[0], new byte[0]);
                        this.icq.c.sendPacket(reply);
                ...
                if (packet instanceof SnacPacket)
                {
                            ...
                            && (snacPacket.getCommand() == SnacPacket.SRV_RATES_COMMAND))
                    {
                        ...
                        SnacPacket reply1 = new SnacPacket(SnacPacket.CLI_ACKRATES_FAMILY,
                                SnacPacket.CLI_ACKRATES_COMMAND, 0x00000000, new byte[0],
                        ...
                        SnacPacket reply2 = new SnacPacket(SnacPacket.CLI_REQINFO_FAMILY,
                                SnacPacket.CLI_REQINFO_COMMAND, 0x00000000, new byte[0], new byte[0]);
                        this.icq.c.sendPacket(reply2);

                        // Send a CLI_REQLISTS packet
                        SnacPacket reply3 = new SnacPacket(SnacPacket.CLI_REQLISTS_FAMILY,
                                SnacPacket.CLI_REQLISTS_COMMAND, 0x00000000, new byte[0], new byte[0]);
                        ...
                        if ((versionId1 == -1) && (versionId2 == -1))
                        {
                            ...
                            SnacPacket reply4 = new SnacPacket(SnacPacket.CLI_REQROSTER_FAMILY,
                                    SnacPacket.CLI_REQROSTER_COMMAND, 0x00000000, new byte[0], new byte[0]);
            ...
            else if (this.state == ConnectAction.STATE_CLI_REQBOS_SENT)
            {
                ...
                if (packet instanceof SnacPacket)
                {
                    ...
                    if (this.srvReplyInfoRcvd && this.srvReplyLocationRcvd && this.srvReplyBuddyRcvd
                            && this.srvReplyIcbmRcvd && this.srvReplyBosRcvd && this.srvReplyListsRcvd
                            && this.srvReplyRosterRcvd)
                    {
                        ...
                        SnacPacket reply1 = new SnacPacket(SnacPacket.CLI_ROSTERACK_FAMILY,
                                SnacPacket.CLI_ROSTERACK_COMMAND, 0x00000000, new byte[0], new byte[0]);
                        this.icq.c.sendPacket(reply1);

                        // Send a CLI_SETUSERINFO packet
                        SnacPacket reply2 = new SnacPacket(SnacPacket.CLI_SETUSERINFO_FAMILY,
                                SnacPacket.CLI_SETUSERINFO_COMMAND, 0x00000000, new byte[0],
                        ...
                        SnacPacket reply3 = new SnacPacket(SnacPacket.CLI_SETICBM_FAMILY,
                                SnacPacket.CLI_SETICBM_COMMAND, 0x00000000, new byte[0], ConnectAction.CLI_SETICBM_DATA);
                        this.icq.c.sendPacket(reply3);
                        ...
                        SnacPacket reply5 = new SnacPacket(SnacPacket.CLI_READY_FAMILY, SnacPacket.CLI_READY_COMMAND,
                                0x00000000, new byte[0], ConnectAction.CLI_READY_DATA);
                        this.icq.c.sendPacket(reply5);

                        // Send a CLI_TOICQSRV/CLI_REQOFFLINEMSGS packet
                        ToIcqSrvPacket reply6 = new ToIcqSrvPacket(0x00000000, this.uin,
                                ToIcqSrvPacket.CLI_REQOFFLINEMSGS_SUBCMD, new byte[0], new byte[0]);
                        this.icq.c.sendPacket(reply6);
            ...
            else if (this.state == ConnectAction.STATE_CLI_REQOFFLINEMSGS_SENT)
            {
                ...
                if (packet instanceof FromIcqSrvPacket)
                {
                    ...
                    if (fromIcqSrvPacket.getSubcommand() == FromIcqSrvPacket.SRV_DONEOFFLINEMSGS_SUBCMD)
                    {
                        ...
                        ToIcqSrvPacket reply = new ToIcqSrvPacket(0x00000000, this.uin,
